"""Integration tests for Hybrid Retrieval with FAISS, BM25, RRF, and RAG service (Phase 4.8)."""
...
class TestHybridRetrievalIntegration:
    """Integration test suite for hybrid search and downstream RAG pipeline."""

    @pytest.fixture
    def indexed_hybrid_retriever(self) -> HybridRetriever:
        dim = 1024
        chunks = [
            _make_chunk(
                "chk_synth_0001",
                "clinical_note_0001",
                "Discharge Medications: Metformin 500mg BD, Glimepiride 1mg OD, Atorvastatin 20mg HS. Patient diagnosed with T2DM.",
            ),
            _make_chunk(
                "chk_synth_0002",
                "clinical_note_0002",
                "Discharge summary for CAD with STEMI. Patient prescribed Aspirin 75mg, Clopidogrel 75mg, and Ramipril 2.5mg.",
            ),
            _make_chunk(
                "chk_synth_0003",
                "clinical_note_0003",
                "Patient with acute appendicitis underwent laparoscopic appendectomy. Prescribed Cefixime 200mg BD.",
            ),
        ]

        # 1. Setup FAISS Dense RetrievalEngine
    ...
    def test_end_to_end_hybrid_evaluation(self, indexed_hybrid_retriever: HybridRetriever) -> None:
        """Verify HybridRetriever integrates seamlessly with RAGEvaluator."""
        evaluator = RAGEvaluator(retrieval_engine=indexed_hybrid_retriever, k_values=(1, 3))
        cases = DEFAULT_GROUND_TRUTH_CASES[:2]  # Test subset of cases
    ...
    def test_rag_pipeline_compatibility(self, indexed_hybrid_retriever: HybridRetriever) -> None:
        """Verify hybrid results flow through ContextBuilder, PromptBuilder, and RAGService."""
        context_builder = ContextBuilder(max_chunks=3, max_context_chars=4000)
        prompt_builder = PromptBuilder()
        fake_llm = FakeLLMProvider()

        rag_service = RAGService(
            retrieval_engine=indexed_hybrid_retriever,
            context_builder=context_builder,
            prompt_builder=prompt_builder,
            llm_provider=fake_llm,
        )

        answer = rag_service.answer("What medications were prescribed for Metformin?")
        assert answer.answer_text is not None
        assert len(answer.source_references) > 0
        assert answer.model_name == "fake-llm-v1"
        assert answer.num_retrieved > 0
